...
def ask_for_time():
    ...
    return int(hour_answer['hour']), int(minute_answer['minute']), int(
...
def place_order_at_time():
    ...
    for entry in favorite_item:
        ...
        price = item['item_price']['minor_units'] / \
        (10 ** item['item_price']['decimals'])
        currency_code = item['item_price']['code']
    ...
    choices = {
        # f"{product['name']} - {product['seller_name']} - {product['seller_address']} - {product['price']} {product['currency_code']}":
        # product_id
        # for product_id, product in item_dict.items()
        f"{product['name']} - {product['seller_name']} - {product['price']} {product['currency_code']}":
        product_id
        for product_id, product in item_dict.items()
    }
    ...
    now = datetime.now()
    target_time = now.replace(hour=hour,
                              minute=minute,
                              second=second,
                              microsecond=0)
    if now > target_time:
    ...
    while k == 1:
        try:
            ...
            pay = client.pay_order(order_id=order["id"],
                                   card_data={
                                       "card": card_number,
                                       "cvv": cvv,
                                       "month": month,
