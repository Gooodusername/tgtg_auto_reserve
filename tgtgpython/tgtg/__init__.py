...
class TgtgClient:
    ...
    def _refresh_token(self):
        if (self.last_time_token_refreshed and
            (datetime.datetime.now() - self.last_time_token_refreshed).seconds
    ...
    def login(self):
        if not (self.email
                or self.access_token and self.refresh_token and self.cookie):
            raise TypeError(
        ...
        if self._already_logged:
            ...
            for _ in range(
                    2):  # doing twice the request to try to bypass captcha
                response = self.session.post(
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                if first_login_response["state"] == "TERMS":
                    ...
                    raise TgtgLoginError(response.status_code,
                                         response.content)
            else:
                if response.status_code == HTTPStatus.TOO_MANY_REQUESTS:
                    raise TgtgAPIError(response.status_code,
